...
export function TodoList({
  ...
  if (todos.length === 0) {
    return (
      <Card className="min-h-full border-border/80 shadow-xs">
        <CardHeader>
          <CardTitle>执行清单</CardTitle>
          <CardDescription>
            当前筛选下的待办会在这里按日期分组。
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card className="min-h-full border-border/80 shadow-xs">
      <CardHeader>
        <CardTitle>执行清单</CardTitle>
        <CardDescription>
          {activeCategoryLabel} 下共有 {todos.length} 项待办。
        </CardDescription>
      </CardHeader>
      <CardContent>
        <section className="space-y-5" aria-label="待办事项列表">
            ...
            <section key={date} className="space-y-2">
              ...
              <ul className="space-y-2">
                {dateTodos.map((todo) => (
                  <li
                    key={todo.id}
                    className="grid grid-cols-[auto_1fr_auto] items-center gap-3 border border-border/80 bg-card px-3 py-2.5 shadow-xs transition-colors hover:bg-muted/35"
                  >
                    <Checkbox
